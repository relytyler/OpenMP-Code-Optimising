...
int propagate(const t_param params, t_speed* cells, t_speed* tmp_cells, int jj, int ii);
int rebound(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles, int jj, int ii);
int collision(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles, int jj, int ii);
...
float timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
  ...
  for (int jj = 0; jj < (params.ny); jj++)
  {
    ...
    {
      // propagate(params, cells, tmp_cells, jj, ii);
      ...
      ** scratch space grid */
      speedsTemp[0] = cells[ii + jj*params.nx].speeds[0]; /* central cell, no movement */
      speedsTemp[1] = cells[x_w + jj*params.nx].speeds[1]; /* east */
      speedsTemp[2] = cells[ii + y_s*params.nx].speeds[2]; /* north */
      speedsTemp[3] = cells[x_e + jj*params.nx].speeds[3]; /* west */
      speedsTemp[4] = cells[ii + y_n*params.nx].speeds[4]; /* south */
      speedsTemp[5] = cells[x_w + y_s*params.nx].speeds[5]; /* north-east */
      speedsTemp[6] = cells[x_e + y_s*params.nx].speeds[6]; /* north-west */
      speedsTemp[7] = cells[x_e + y_n*params.nx].speeds[7]; /* south-west */
      speedsTemp[8] = cells[x_w + y_n*params.nx].speeds[8]; /* south-east */
      

      // rebound(params, cells, tmp_cells, obstacles, jj, ii);
      /* if the cell contains an obstacle */
      if (obstacles[jj*params.nx + ii])
      {
        /* called after propagate, so taking values from scratch space
        ** mirroring, and writing into main grid */
        tmp_cells[ii + jj*params.nx].speeds[1] = speedsTemp[3];
        tmp_cells[ii + jj*params.nx].speeds[2] = speedsTemp[4];
        tmp_cells[ii + jj*params.nx].speeds[3] = speedsTemp[1];
        tmp_cells[ii + jj*params.nx].speeds[4] = speedsTemp[2];
        tmp_cells[ii + jj*params.nx].speeds[5] = speedsTemp[7];
        tmp_cells[ii + jj*params.nx].speeds[6] = speedsTemp[8];
        tmp_cells[ii + jj*params.nx].speeds[7] = speedsTemp[5];
        tmp_cells[ii + jj*params.nx].speeds[8] = speedsTemp[6];
      }
      else{
        // collision(params, cells, tmp_cells, obstacles, jj, ii);
        /* compute local density total */
        float local_density = 0.f;

        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          local_density += speedsTemp[kk];
        }

        /* compute x velocity component */
        float u_x = (speedsTemp[1]
                      + speedsTemp[5]
                      + speedsTemp[8]
                      - (speedsTemp[3]
                         + speedsTemp[6]
        ...
        for (int kk = 0; kk < NSPEEDS; kk++)
        {
          tmp_cells[ii + jj*params.nx].speeds[kk] = speedsTemp[kk]
                                                  + params.omega
                                                  * (d_equ[kk] - speedsTemp[kk]);
        }
